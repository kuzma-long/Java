...
> 比亚迪，英雄游戏，百度-2，字节-2，度小满，华为，阿里云

Spring AOP（Aspect-Oriented Programming）是一种面向切面编程的技术，它通过动态代理的方式，在不修改原有代码的基础上，通过代理类拦截方法的调用，并在调用前后加入自定义的逻辑，实现了将横切关注点（如日志、事务、安全等）从核心业务逻辑中分离出来的功能。Spring AOP 的实现方式主要有两种：基于 JDK 动态代理和基于 CGLIB 动态代理。
