...
InnoDB使用多版本并发控制（MVCC）来实现一致性。MVCC基于时间戳，使每个事务能够看到一致性视图。这意味着读取操作只会看到已提交的数据，而写入操作将被视为原子操作，直到提交事务为止。
...
> 蔚来，百度，小米，京东，字节，美团，阿里云，拼多多搜广，度小满，滴滴，快手，万得
...
> 字节，百度，英雄游戏，小米，京东，阿里云，滴滴

1. B+ 树的非叶子节点只存储索引，而不存储数据信息，因此相比 B 树可以存储更多的索引项，使得树的高度更低，查询效率更高。
2. B+ 树的叶子节点都被连接成一个有序链表，因此在范围查询时更加高效，可以直接遍历整个链表。而在 B 树中，需要通过非叶子节点进行查找，查询效率相对较低。
3. B+树查询效率更稳定。B+树每次都必须查询到叶子节点才能找到数据，而B树查询的数据可能不在叶子节点，也可能在，这样就会造成查询的效率的不稳定。
4. B+ 树的所有数据都存储在叶子节点中，因此在数据访问时可以利用局部性原理，将相邻的数据预读到缓存中，提高了查询效率。
5. B+ 树的叶子节点可以通过链表方便地进行数据的插入、删除，而在 B 树中需要进行节点的分裂、合并，实现复杂。
