...
在 JVM 中，堆内存（Heap）通常被分为新生代（Young Generation）、老年代（Old Generation）和永久代（Permanent Generation）三部分。其中，新生代和老年代是 Java 堆内存的两个主要部分。

新生代是一个对象被创建后的初始存放位置，它通常被划分为三个部分：Eden 区、Survivor 0 区和 Survivor 1 区。当一个对象被创建时，它会被放入 Eden 区中。当 Eden 区满时，JVM 会对 Eden 区和 Survivor 0 区中的对象进行垃圾回收，将未被回收的对象移动到 Survivor 1 区中。当 Survivor 1 区也满时，JVM 会将 Survivor 0 区和 Survivor 1 区中的对象进行垃圾回收，并将未被回收的对象移动到另一个 Survivor 区中。当一个对象经过多次垃圾回收后仍然存活时，它会被移动到老年代中。

老年代是 Java 堆内存中存放较长时间存活对象的区域。当一个对象经过多次垃圾回收后，仍然存活时，它会被移动到老年代中。在老年代中，垃圾回收的次数相对较少，因此进行垃圾回收时，需要耗费更长的时间。为了避免频繁进行垃圾回收，JVM 通常会将老年代划分为多个部分，以便在进行垃圾回收时，只回收其中的一部分，从而减少垃圾回收的时间。
...
JVM新生代一般分为Eden空间、From Survivor空间和To Survivor空间，Eden空间用于存放新创建的对象，From Survivor空间和To Survivor空间用于存放存活的对象。大对象一般会直接分配到老年代，以减少新生代的垃圾回收频率。
